<?php

namespace App\Http\Controllers;

use App\Http\Controllers\Controller;

use App\Models\Prize;
use App\Http\Requests\PrizeRequest;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Http\Request;



class PrizesController extends Controller
{
    /**
     * Display a listing of the resource.
     *
     * @return \Illuminate\Contracts\View\View
     */
    public function index()
    {
        $prizes = Prize::all();
        $totalProbability = Prize::sum('probability');

        return view('prizes.index', ['prizes' => $prizes, 'totalProbability' => $totalProbability]);
    }

    /**
     * Show the form for creating a new resource.
     *
     * @return \Illuminate\Contracts\View\View
     */
    public function create()
    {
        return view('prizes.create');
    }

    /**
     * Store a newly created resource in storage.
     *
     * @param  PrizeRequest  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function store(PrizeRequest $request)
    {

        $totalProbability = Prize::sum('probability') + $request->probability;

        $totalProbability = Prize::sum('probability') + $request->probability;
        $remainingPercentage = 100 - Prize::sum('probability');

        if ($totalProbability > 100) {
            return back()->withErrors(['probability' => 'The probability must not be grater then ' . $remainingPercentage . '%']);
        }

        $prize = new Prize;
        $prize->title = $request->input('title');
        $prize->probability = floatval($request->input('probability'));
        $prize->save();

        return redirect()->route('prizes.index')->with('success', 'Prize added successfully.');
    }



    /**
     * Show the form for editing the specified resource.
     *
     * @param  int  $id
     * @return \Illuminate\Contracts\View\View
     */
    public function edit($id)
    {
        $prize = Prize::findOrFail($id);
        return view('prizes.edit', ['prize' => $prize]);
    }

    /**
     * Update the specified resource in storage.
     *
     * @param  PrizeRequest  $request
     * @param  int  $id
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update(PrizeRequest $request, $id)
    {

        $totalProbability = Prize::where('id', '!=', $id)->sum('probability');
        $totalProbabilityAfterNew = Prize::where('id', '!=', $id)->sum('probability') + $request->probability;
        $prize = Prize::findOrFail($id);
        $remainingProbability = 100 - $totalProbability;
        if ($totalProbabilityAfterNew > 100) {
            return back()->withErrors(['probability' => ' The probability field must not be grater then '.$remainingProbability.'.']);
        }
        
        $prize->title = $request->input('title');
        $prize->probability = floatval($request->input('probability'));
        $prize->save();
        return redirect()->route('prizes.index')->with('success', 'Prize updated successfully.');
    }

    /**
     * Remove the specified resource from storage.
     *
     * @param  int  $id
     * @return \Illuminate\Http\RedirectResponse
     */
    public function destroy($id)
    {
        $prize = Prize::findOrFail($id);
        $prize->delete();

        Prize::query()->update([
            'awarded_count' => 0
        ]);

        return redirect()->route('prizes.index')->with('success', 'Prize deleted successfully.');
    }


    public function simulate(Request $request)
    {
        $totalProbability = Prize::sum('probability');
        $totalPrizes = $request->number_of_prizes;
        if ($totalProbability < 100) {
            return back()->withErrors(['probability' => 'The probability must be 100 % for simulation']);
        }
        
        for ($i = 0; $i < $totalPrizes ?? 10; $i++) {
            Prize::nextPrize($totalPrizes);
        }

        return to_route('prizes.index');
    }

    public function reset()
    {
        Prize::query()->update([
            'awarded_count' => 0
        ]);
       
        return to_route('prizes.index');
    }
}
