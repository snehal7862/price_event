...
@push('scripts')
    ...
    <script>
        const totalProbabilities = @json($prizes->sum('probability'));
        if(totalProbabilities === 100){
            const ctx1 = document.getElementById('probabilityChart').getContext('2d');
            const ctx2 = document.getElementById('rewardsChart').getContext('2d');
        }
        const titles = @json(\App\Models\Prize::all()->pluck('probability','title'));
        const results =  @json(\App\Models\Prize::all()->pluck('awarded_count','title'));
    </script>
    @vite('resources/assets/js/prize.js')
@endpush
